...
def setup_logging():
    ...
    # Formatierung für alle Logs
    formatter = logging.Formatter(
        '%(asctime)s - %(name)s - %(levelname)s - %(threadName)s - %(message)s'
    )
    
    # Datei-Handler für alle Logs
    main_log = logging.handlers.RotatingFileHandler(
        os.path.join(log_dir, 'tuxguard.log'),
...
def preprocess_data(data):
    ...
    def calculate_gesture_features(positions, velocities):
        ...
        return pd.DataFrame(gestures).mean().to_dict() if gestures else {
            'complexity': 0, 'smoothness': 0, 'efficiency': 0, 'peak_velocity': 0
        }
    ...
    try:
        ...
        try:
            ...
            timestamps = data[:, 2].astype(np.float32)
        ...
        if np.any(np.diff(timestamps) <= 0):
            ...
            raise ValueError("Zeitstempel müssen streng monoton steigend sein")
            
        logger.debug("Basisdaten erfolgreich extrahiert und validiert")
        
        # Basis-Features
        time_diffs = np.diff(timestamps, prepend=timestamps[0])
        logger.debug(f"Zeitdifferenzen berechnet: Min={time_diffs.min():.3f}s, Max={time_diffs.max():.3f}s")
        ...
        click_density = click_count / (timestamps[-1] - timestamps[0]) if len(timestamps) > 1 else 0
...
def incremental_training():
    ...
    try:
        ...
        # Erstelle Labels (alle 1, da nur vertrauenswürdige Daten gespeichert wurden)
        ...
        precision = history.history.get('precision', [0])[0]
        recall = history.history.get('recall', [0])[0]
